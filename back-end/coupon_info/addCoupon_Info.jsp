...
<%
	Coupon_InfoVO coupon_infoVO = (Coupon_InfoVO) request.getAttribute("coupon_infoVO");
%>
		...
		<div class="row">
			<div class=col-12>
				<table id="table-1">
					<tr>
						<td>
							<h3>優惠券資料新增 - addCoupon_List.jsp</h3>
							<h4>
					...
					<table>
						<tr>
							...
							<td><input type="TEXT" name="product_name" size="45"
								value="<%=(coupon_infoVO == null) ? "Starbucks Latte" : coupon_infoVO.getProduct_name()%>" /></td>
						</tr>
						<tr>
							<td>商品品牌名稱:<font color=red><b>*</b></font></td>
							<td><input type="TEXT" name="product_brand" size="45"
								value="<%=(coupon_infoVO == null) ? "Starbucks" : coupon_infoVO.getProduct_brand()%>" /></td>
						</tr>
						<tr>
							<td>兌換條件:<font color=red><b>*</b></font></td>
							<td><input type="TEXT" name="exc_condition" size="45"
								value="<%=(coupon_infoVO == null) ? "150" : coupon_infoVO.getExc_condition()%>" /></td>
						</tr>
						<tr>
							<td>兌換期限:<font color=red><b>*</b></font></td>
							<td><input type="TEXT" name="exe_deadline" size="45"
								value="<%=(coupon_infoVO == null) ? "180" : coupon_infoVO.getExe_deadline()%>" /></td>
						</tr>
						<tr>
							<td>商品圖片:</td>
							<td><input type="File" name="product_photo"
								id="product_photo" accept="image/*" /> <%-- 		value="<%= (coupon_infoVO==null)? "null" : coupon_infoVO.getProduct_photo()%>" --%>
							</td>
						</tr>
						<tr>
							<td></td>
						...
						<tr>
							...
							<td><textarea name="product_context"><%=(coupon_infoVO == null) ? "星巴克那堤" : coupon_infoVO.getProduct_context()%></textarea>
								<!-- 			<input type="TEXT" name="product_context" id="product_context" value="" /> -->
							</td>
						</tr>
						<tr>
							<td>優惠類別:<font color=red><b>*</b></font></td>
							<td><input type="TEXT" name="cpon_type" size="45"
								value="<%=(coupon_infoVO == null) ? "美食" : coupon_infoVO.getCpon_type()%>" /></td>
						</tr>
						<jsp:useBean id="areaListSvc" scope="page"
							class="com.area_list.model.AreaListService" />
						<tr>
							<td>可兌換地區:<font color=red><b>*</b></font></td>
						...
						<tr>
							...
							<td><input type="TEXT" name="exc_count" size="45"
								value="<%=(coupon_infoVO == null) ? "9999" : coupon_infoVO.getExc_count()%>" /></td>
						</tr>
						<tr>
							<td>已被兌換次數:<font color=red><b>*</b></font></td>
							<td><input type="TEXT" name="exced_count" size="45"
								value="<%=(coupon_infoVO == null) ? "0" : coupon_infoVO.getExced_count()%>" /></td>
						</tr>
						<tr>
							<td>商品上架狀態:<font color=red><b>*</b></font></td>
							<%-- 		value="<%= (coupon_infoVO==null)? "下架" : coupon_infoVO.getShelf_sts()%>" --%>
							<td><input type="radio" name="shelf_sts" value="下架"
								${(coupon_infoVO.shelf_sts==null || coupon_infoVO.shelf_sts=='下架')? 'checked':''} />下架
								<input type="radio" name="shelf_sts" value="上架"
								${(coupon_infoVO.shelf_sts=='上架')? 'checked':''} />上架</td>
						</tr>
						<tr>
							<td>商品刊登日期:<font color=red><b>*</b></font></td>
...
<%
	...
	try {
		onshelf_date = coupon_infoVO.getOnshelf_date();
	} catch (Exception e) {
...
<%
	...
	try {
		cpon_offshelf_date = coupon_infoVO.getCpon_offshelf_date();
	} catch (Exception e) {
		cpon_offshelf_date = new java.sql.Date(System.currentTimeMillis());
	}
%>
...
// 	       step: 1,                //step: 60 (這是timepicker的預設間隔60分鐘)
		   ...
		   value: '<%=onshelf_date%>', // value:   new Date(),
        ...
        $('#f_date2').datetimepicker({
 	       timepicker:false,
 	       format:'Y-m-d',
 	       value: (<%=cpon_offshelf_date%> == null)? '': '<%=cpon_offshelf_date%>
	',
			});
