...
<%
	...
	List<Member_InfoVO> friendList = memSvc.getFriends(thisMem_VO.getMem_id());
	...
	List<Member_HobbyVO> memHobVO = memHobSvc.findHobCode(((Member_InfoVO)session.getAttribute("member_infoVO")).getMem_id());
								...
								<form method="post"
									...
									<c:forEach var="Hobby_ListVO" items="${hobVO}">
										<label><input type="checkbox" name="hobby"
											value="${Hobby_ListVO.hob_code}">${Hobby_ListVO.hob}</label><br>

									</c:forEach>

									<input type="hidden" name="Member" value="${thisMem}">
									<input type="hidden" name="action" value="addHobby">
									<input type="submit" value="送出過濾條件" style="position:absolute;">
									</div>
									</form>
									...
									<c:forEach var="memHobVO" items="${memHobVO}">
										<c:forEach var="Hobby_ListVO" items="${hobVO}">
											...
											<label><input type="checkbox" name="delHob" value="${Hobby_ListVO.hob_code}">
											${Hobby_ListVO.hob}</label><br>
											</c:if>
										</c:forEach>	
									</c:forEach>
...
<!-- 改的			 -->
				<aside>
					...
					<a href="<%=request.getContextPath()%>/front-end/profile/introduction.jsp?Member=${thisMem}">
						<i class="far fa-address-card" aria-hidden="true"></i> 個人簡介
					</a>
					 <a	href="<%=request.getContextPath()%>/front-end/profile/listAllPost.jsp?Member=${thisMem}">
						<i class="far fa-newspaper" aria-hidden="true"></i> 個人動態
					</a> 
					<a href="<%=request.getContextPath()%>/front-end/profile/friendList.jsp?Member=${thisMem}"> 
				...
				<table>
					...
					<article class="paper">
						...
						<c:forEach var="friend" items="${friendList}">
							<div>
								<%
									Member_InfoVO friend = (Member_InfoVO) pageContext.getAttribute("friend");
								%>
								<a	href="<%=request.getContextPath()%>/front-end/profile/introduction.jsp?Member=<%=friend.getMem_id()%>">
								<img id="gs_big_pic_img"
						src="<%=request.getContextPath()%>/back-end/post/post?action=display_pic&Member=${friend.mem_id}"
