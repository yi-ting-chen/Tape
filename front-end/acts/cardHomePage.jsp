...
<%
	...
	//List<ActsVO> list = actsSvc.getAll();//取得所有活動 包含過時 已參加過
	List<ActsVO> list = actsSvc.get_all_by_time();//不包含過時的活動
	DateFormat sdf = new SimpleDateFormat("yyyy/MM/dd HH:mm");

	//取出會員資料
	Member_InfoVO member_infoVO = (Member_InfoVO) session.getAttribute("member_infoVO");
	if (member_infoVO != null) {
		list = actsSvc.get_all_can_join(member_infoVO.getMem_id());//有登入下 去掉已報名的活動
...
<!-- 相片檢視器 CSS -->
<!-- <link rel="stylesheet" -->
<%-- 	href="<%=request.getContextPath()%>/front-end/css/viewer/viewer.min.css"> --%>
<!-- 相片檢視器 CSS -->
...
<!-- RWD 區塊切割提示 CSS-->
<%-- 	<link rel="stylesheet" href="<%= request.getContextPath() %>/front-end/acts/css/style.css"> --%>

<!-- 卡片活動 CSS -->
<link rel="stylesheet"
	href="<%=request.getContextPath()%>/front-end/acts/css/myCard.css">
				...
				<form method="post"
					...
					<select class="custom-select my-1 mr-sm-2"
					 ...
					 <label for="f_date1">起算時間</label> <input name="time1" id="f_date1"
						type="text"> <label for="f_date2">結束時間</label> <input
						name="time2" id="f_date2" type="text"> <br> <label
						id="area"> 地點</label>

					<jsp:useBean id="alSvc" scope="page"
						class="com.area_list.model.AreaListService" />
...
<%
	...
	time1 = new java.sql.Timestamp(System.currentTimeMillis());
%>
